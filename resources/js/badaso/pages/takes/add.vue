              ...
              <vs-col v-for="(dataRow, rowIndex) in dataType.dataRows" :key="rowIndex"
                ...
                <template v-if="dataRow.add == 1">
                  ...
                  <badaso-number v-if="dataRow.type == 'number'" :label="dataRow.displayName"
                    :placeholder="dataRow.displayName" v-model="dataRow.value" size="12" :alert="errors[$caseConvert.stringSnakeToCamel(dataRow.field)]
                      "></badaso-number>
                  <badaso-url v-if="dataRow.type == 'url'" :label="dataRow.displayName" :placeholder="dataRow.displayName"
                  ...
                  <badaso-time v-if="dataRow.type == 'time'" :label="dataRow.displayName"
                    :placeholder="dataRow.displayName" v-model="dataRow.value" value-zone="local" size="12" :alert="errors[$caseConvert.stringSnakeToCamel(dataRow.field)]
                      "></badaso-time>
                  ...
                  <badaso-datetime v-if="dataRow.type == 'datetime_readonly'" :label="dataRow.displayName"
                    :placeholder="dataRow.displayName" v-model="dataRow.value" value-zone="local" size="12" :style="'pointer-events:none'" :alert="errors[$caseConvert.stringSnakeToCamel(dataRow.field)]
                      "></badaso-datetime>
                  ...
                  <badaso-upload-file v-if="dataRow.type == 'upload_file'" :label="dataRow.displayName"
                    ...
                    :shares-only="dataRow.details.type == 'shares-only'" size="12" :alert="errors[$caseConvert.stringSnakeToCamel(dataRow.field)]
                      "></badaso-upload-file>
                  <badaso-upload-file-multiple v-if="dataRow.type == 'upload_file_multiple'" :label="dataRow.displayName"
                    :placeholder="dataRow.displayName" v-model="dataRow.value"
                    :private-only="dataRow.details.type == 'private-only'"
                    :shares-only="dataRow.details.type == 'shares-only'" size="12" :alert="errors[$caseConvert.stringSnakeToCamel(dataRow.field)]
                  ...
                  <badaso-select-multiple v-if="dataRow.type == 'select_multiple'" :label="dataRow.displayName"
                    :placeholder="dataRow.displayName" v-model="dataRow.value" size="12" :alert="errors[$caseConvert.stringSnakeToCamel(dataRow.field)]
                  ...
                  <badaso-select v-if="dataRow.type == 'relation' &&
                    ...
                    " :label="dataRow.displayName" :placeholder="dataRow.displayName" v-model="dataRow.value" size="12"
                    :items="relationData[
                  ...
                  <badaso-select v-if="dataRow.type == 'relation_readonly' &&
                    dataRow.relation.relationType == 'belongs_to'
                    " :label="dataRow.displayName" :placeholder="dataRow.displayName" v-model="dataRow.value" :style="'pointer-events:none;'" size="12"
                    :items="relationData[
                      $caseConvert.stringSnakeToCamel(
                        dataRow.relation.destinationTable
                      )
  ...
  methods: {
    submitForm() {
      ...
      if (Object.values(dataRows).length == 0) {
        ...
        //     gudang.capacityLeft += amountOfTake;
        //     console.log(gudang, 'jambi')
      ...
      // start request
      this.$openLoader();
      this.amountOfTake = dataRows.amount_of_take;
      console.log(this.amountOfTake,"amountoftake")
      this.$api.badasoEntity.read({
        slug: 'stocks',
        id: dataRows.stock_id
      }).then((response) => {
        let data = response.data
        if (data.amountOfStock >= this.amountOfTake) {
          data.amountOfStock -= this.amountOfTake;
          this.$api.badasoEntity.edit({
            slug: 'stocks',
            data: data,
          }).then((response) => {
            this.$api.badasoEntity
              .add({
                ...
                id: data.branchId
                  ...
                  let gudang = response.data
                  console.log(typeof(gudang.capacityLeft), "kapasitas left")
                  gudang.capacityLeft += parseInt(this.amountOfTake);
                  console.log(gudang.capacityLeft, 'hasil kappasitas')
                  this.$api.badasoEntity.edit({
                    slug: 'branches',
                    data: gudang
                  })
          ...
          this.$vs.notify({
            ...
            text: 'Jumlah ambil melebihi stock',
            color: "danger",
          });
        }
      })
    ...
    async getDataType() {
      ...
      try {
        ...
        const response_stock = await this.$api.badasoEntity.browse({
          slug: 'stocks'
        })
        console.log(response_stock.data.data[0].itemName, 'bang')

        // const user = await this.$api.badasoCrud.readBySlug({
        //   slug: 'badaso-users',
        // }) 
        // const response_stocks = await this.$api.badasoEntity.browse({
        //   slug: 'stocks',
        // });
        // let table_stocks = response_stocks.data.data
        // let stock = {}
        // for(let stocks of table_stocks){
        //   stock[stocks.id] = stocks.amount_of_stock;
        // }
        // this.stock = stock
        // console.log = (this.stock, "data"); 
        
        const response_users = await this.$api.badasoAuthUser.user({})
        console.log(response_users.data.user, 'bang')
        this.$closeLoader();
        ...
        const dataRows = response.data.crudData.dataRows.map((data) => {
          ...
          if(data.field == "user_id"){
            data.value = response_users.data.user.id
          ...
          if(data.field == "date_of_take"){
            data.value = new Date();
            data.type = "datetime_readonly";
          }


          // if(data.field == "stock_id"){
          //   // console.log = (response_stocks.data, "anjay");
          //   data.value = response_stock.data.data[0];
          //   data.type = "relation"
          // }
    ...
    getRelationDataBySlug() {
      ...
      this.$api.badasoTable
        ...
        .then((response) => {
          ...
          console.log(this.relationData.stocks, 'relasi')
        })
        .catch((error) => {
          if (error.status == 503) {
            this.isMaintenance = true;
